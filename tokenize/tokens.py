def tokenize(source, tokversion, listbox, lemmat=True, message=True):    
    import re
    import nltk
    ...
    dict_titles = corpora.dictionary.Dictionary.load('docs/corpora/stopwords-short.dict')
    ...
    texts_file = open('docs/provisional/tokens'+tokversion+'.txt', 'a')
    ...
    for n in range(len(abstracts)):
        if (n == int(len(abstracts)*0.2) or n == int(len(abstracts)*0.4) or n == int(len(abstracts)*0.6) or n == int(len(abstracts)*0.8) or n == int(len(abstracts)-1)):
            if message is True:
                listboxmes.listboxmsg_1half(n,len(abstracts),line,listbox, 0)
            else:
                listboxmes.listboxmsg_2half_v2(n,len(abstracts),line,listbox, 0)
